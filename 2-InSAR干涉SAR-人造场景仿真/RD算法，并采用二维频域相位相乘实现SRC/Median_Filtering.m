...
% 本函数原理基于：      “回转中值滤波”
% P.S. 中值即为中位数：表示从小到大排列的一组数中，中间的那个数字（或者中间的两个数的平均）
% 
% 本函数用来对去平地相位后的干涉相位做“回转中值滤波”
% 该方法参考：保铮《雷达成像技术》第8章，8.5.2节。
%            详见第 306 页 相关内容
%
% 函数输入值：
% 1） PHY_s_after_flat_earth      代表去平地相位后的干涉相位（即缠绕相位）；
% 2） window_M 和 window_N        表示“回转中值滤波”所选定的窗口大小；
% 函数返回值：
% 1）PHY_s_after_median_filtering   表示“回转中值滤波”后的干涉相位（也是缠绕相位）；
%
% 本程序截止至：2014.12.18. 14:15 p.m.
...
disp('正在进行“回转中值滤波”，请等待');
h = waitbar(0,'正在进行“回转中值滤波”，请等待');
for pp = 1:Naz
    for qq = 1:Nrg
        ...
        tmp2 = reshape(tmp,1,[]);      % 整理成一行X列的一组数，便于求中位数；
